...
# -*- coding: utf-8 -*-
"""
Báo cáo tỷ lệ từ chối khi tắt watcher (nowatcher)
- Đọc results.jsonl (case + summary theo loại nhiễu, watcher=off)
- Sinh result.html với Chart.js tương tác, chế độ Sáng/Tối,
  Chia đôi kéo thả và Ẩn/Hiện tiêu đề (giống bench_ram).
...
def _parse_line_lenient(line: str):
    """json.loads trước; regex fallback cho path Windows chưa escape."""
    try:
...
def main():
    ...
    if not summaries:
        raise ValueError("results.jsonl không có dòng type=summary nào.")
...
<title>Tỷ lệ Từ chối khi Tắt Watcher (nowatcher)</title>
...
<div class="container">
  ...
  <header class="header">
    <div class="header-titles">
      <h1>
        Tỷ lệ Từ chối khi Tắt Watcher (nowatcher)
      </h1>
      <div class="header-sub">
        Tạo lúc: {now_str} &bull; Nguồn: {JSONL.name} &bull; {total_cases} cases / {len(summaries)} loại nhiễu, watcher=off
      </div>
    </div>
  </header>
  ...
  <main class="main-content" id="main-content">
    ...
    <div class="panel panel-chart" id="panel-chart">
      <div class="panel-header">
        <span class="panel-title">📊 Chấp nhận vs Từ chối theo Dạng Nhiễu (watcher tắt)</span>
        <div class="controls">
      ...
      <section class="kpi-grid kpi-inside-chart">
        <div class="kpi-card blue-card">
          <div class="kpi-label">Tổng số case (watcher off)</div>
        ...
        <div class="kpi-card green-card">
          ...
          <div class="kpi-subtext">không có quan sát watcher</div>
        </div>
        <div class="kpi-card amber-card">
          <div class="kpi-label">Can thiệp thành công</div>
          <div class="kpi-value">{sum(int(s.get("applied", 0)) for s in summaries)}/{sum(int(s.get("scheduled", 0)) for s in summaries)}</div>
          <div class="kpi-subtext">applied / scheduled</div>
        </div>
      </section>
    ...
    <div class="panel panel-table" id="panel-table">
      <div class="panel-header">
        <span class="panel-title">📋 Chi tiết ({total_cases} cases, watcher off)</span>
      </div>
...
"""
    ...
    print(f"[XONG] Báo cáo nowatcher tại: {HTML_OUT}")
    print(
        f"Tóm tắt: {total_cases} cases (watcher off) | "
        f"TB từ chối {avg_rate:.2f}% | từ chối {total_rejected}/{total_cases}"
    )
