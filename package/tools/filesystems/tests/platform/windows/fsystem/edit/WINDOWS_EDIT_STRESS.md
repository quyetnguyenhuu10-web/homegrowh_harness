...
```
cmake -S . -B build -DFILESYSTEMS_BUILD_TESTS=ON -DFILESYSTEMS_BUILD_WINDOWS_STRESS_TESTS=ON
cmake --build build --config Debug --target windows_edit_stress_bench
```
...
| ---------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `smoke`    | `node tests/platform/windows/fsystem/edit/windows_edit_stress/run_all.js --profile smoke --build-dir build`    | Mặc định, chạy nhanh                         |
| `standard` | `node tests/platform/windows/fsystem/edit/windows_edit_stress/run_all.js --profile standard --build-dir build` | Như CI (`windows_edit_stress_standard_test`) |
| `extreme`  | `node tests/platform/windows/fsystem/edit/windows_edit_stress/run_all.js --profile extreme --build-dir build`  | Chỉ chạy tay                                 |
...
- `lock`: giữ exclusive lock **trước** khi edit chạy; edit phải bị từ chối (`ERROR_SHARING_VIOLATION`).
...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Benchmark edit       | `node tests/platform/windows/fsystem/edit/windows_edit_stress/bench/bench.js --profile smoke --build-dir build`                                                                                                        |
| Phân tích kích thước | `node tests/platform/windows/fsystem/edit/windows_edit_stress/size_analysis/size_analysis.js --profile smoke`   (gộp `results.jsonl` của bench, không chạy exe riêng; 2 scatter `old_data`/`new_data` size → duration) |

Chạy toàn bộ:

`node tests/platform/windows/fsystem/edit/windows_edit_stress/run_all.js --profile smoke --build-dir build`
...
Mỗi executable C++ ghi result schema ổn định vào `bench/results/results.jsonl`, kèm `schema.json` và `summary.json`. File JS chỉ đọc schema đó để tạo `report.html`, nên có thể thay đổi cách trình bày HTML/biểu đồ mà không phải sửa đầu vào kết quả của C++.
