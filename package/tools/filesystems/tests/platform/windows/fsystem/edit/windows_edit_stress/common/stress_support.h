...
#ifndef _WIN32
#error "Windows edit stress support is Windows-only"
#endif
...
namespace windows_edit_stress
{
    ...
    inline constexpr DWORD lock_hold_milliseconds = 5000;
    ...
    {
        std::vector<char> buffer(io_buffer_capacity, value);
        ...
        while (count != 0)
        {
            ...
            hash_bytes(hash, buffer.data(), bytes);
            count -= bytes;
        }
    }
    ...
    ) noexcept
    {
        ...
        // istreambuf_iterator may finish at EOF without setting eofbit.
        // badbit is the reliable signal for an actual read failure here.
        if (input.bad())
        {
    ...
    {
        if (spec.exact_input && !spec.expected_content.empty())
        {
            std::string actual;
    ...
    inline bool verify_initial_content(
        const edit_case_spec& spec,
        std::uint32_t& error
    ) noexcept
    {
    ...
    )
    {
        ...
        try
        {
            ...
            else
            {
                ...
                const std::string expected_note =
                    spec.expected == "success"
                        ? "none"
                        : spec.expected == "not_found"
                            ? "old_data_not_found"
                            : spec.expected == "overlap"
                ...
                const bool edit_ok =
                    ...
                    edit_result.note ==
                        (expected_note == "none"
                            ...
                            : expected_note == "old_data_not_found"
        ...
        if (!value.keep_files)
        {
            ...
            (void)std::filesystem::remove_all(spec.path, error);
        }
    ...
    ) noexcept
    {
        ...
        constexpr char marker[] = "INTERFERENCE";
        DWORD written = 0;
    ...
    {
        if (DeleteFileW(spec.edit.path.c_str()))
            return true;
    ...
    )
    {
        ...
        try
        {
            ...
            else
            {
                ...
                result.lock_at_replace =
                    spec.action == interference_kind::lock &&
                    observation.succeeded &&
                ...
                if (spec.lock_before_edit)
                {
                    result.content_ok = verify_initial_content(
                        spec.edit,
                        verification_error
                    );
                    result.outcome =
                    ...
                    result.detail = result.outcome == "pass"
                        ? "exclusive lock held before edit; source open rejected"
                        : "expected ERROR_SHARING_VIOLATION before replacement";
                }
                ...
                {
                    if (!edit_result.replace_attempted)
                    {
                        result.outcome = "fail";
                        result.detail = "replacement was not attempted";
                    }
                    else if (result.lock_at_replace)
                    {
                        result.outcome = "pass";
                        result.detail =
                            "exclusive lock overlapped the final replacement";
                    }
                    else
                    {
                        result.outcome = "inconclusive";
                        result.detail =
                            "lock overlapped edit but not final replacement";
                    }
                }
                else
                {
                    const bool edit_observed_interference =
                        ...
                        edit_result.note == fsystem::EditNote::old_data_not_found;

                    result.outcome = edit_observed_interference
                        ? "pass"
                        : "fail";
                    result.detail = edit_observed_interference
                        ? "edit reported or observed the concurrent interference"
        ...
        {
            std::error_code error;
            (void)std::filesystem::remove_all(spec.edit.path, error);
    ...
    public:
        ...
        {
            const json record = to_json(result);
            jsonl_ << record.dump() << '\n';
            jsonl_.flush();
