...
namespace
{
        ...
        try
        {
            ...
            else
            {
                if (spec.lock_before_edit)
                {
                    ...
                    {
                        interference.succeeded = true;
                        interference.success_time = clock_type::now();
                    }
                    else
                    {
                        interference.first_error = lock_error;
                    }
                }

                if (
                    spec.action != interference_action::none &&
                ...
                if (spec.expected == "lock_before_edit")
                {
                    ...
                    const bool rejected_before_replace =
                        edit_result.error == ERROR_SHARING_VIOLATION &&
                        !edit_result.replace_attempted;

                    result.outcome =
                        rejected_before_replace &&
                        result.content_ok
                            ? "pass"
                            : "fail";
                    ...
                    {
                        result.detail =
                            "exclusive lock was held before edit; "
                            "source open was rejected";
                    }
                    else
                    {
                        result.detail =
                            "expected ERROR_SHARING_VIOLATION before "
                            "ReplaceFileW; verification_error=" +
                            std::to_string(verification_error);
                    }
                }
                else if (spec.expected == "success")
                {
                ...
                else if (spec.action == interference_action::lock)
                {
                    ...
                    else if (!result.interference_in_window)
                    {
                        result.outcome = "inconclusive";
                        result.detail =
                            "exclusive lock was acquired after edit completed";
                    }
                    else if (!result.lock_at_replace)
                    {
                        result.outcome = "inconclusive";
                        result.detail =
                            "exclusive lock did not overlap ReplaceFileW";
                    }
                    else
                    {
                        result.outcome = "pass";
    ...
    )
    {
        ...
        {
            case_spec spec;
            spec.id = "interference-lock-before-edit";
            spec.scenario = "interference_matrix";
            spec.path = cases_directory /
                "interference-lock-before-edit.bin";
            spec.action = interference_action::lock;
            spec.expected = "lock_before_edit";
            spec.original_size = matrix_size;
            spec.marker_offset = middle_marker;
